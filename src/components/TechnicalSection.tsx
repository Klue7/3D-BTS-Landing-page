...
export function TechnicalSection() {
  ...
  const handleDownload = () => {
    ...
    link.href = '#';
    link.download = 'specification-sheet.pdf';
    document.body.appendChild(link);
    // link.click(); // Commented out to prevent actual navigation in preview
    document.body.removeChild(link);
    alert('Specification sheet download started.');
  };
    ...
    <section id="technical-spotlight" className="relative w-full h-screen bg-[#050505] flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
        <div className="w-[300px] h-[300px] md:w-[600px] md:h-[600px] rounded-full border border-white/20 relative">
          ...
          {[0, 45, 90, 135, 180, 225, 270, 315].map((deg) => (
            <div key={deg} className={`absolute top-0 left-1/2 w-px h-2 md:h-4 origin-bottom md:hidden ${activeCategory === 'cladding-tiles' ? 'bg-[#22c55e]' : 'bg-[#eab308]'}`} style={{ transform: `translateX(-50%) rotate(${deg}deg) translateY(-150px)` }}></div>
          ))}
          {[0, 45, 90, 135, 180, 225, 270, 315].map((deg) => (
            <div key={`md-${deg}`} className={`hidden md:block absolute top-0 left-1/2 w-px h-4 origin-bottom ${activeCategory === 'cladding-tiles' ? 'bg-[#22c55e]' : 'bg-[#eab308]'}`} style={{ transform: `translateX(-50%) rotate(${deg}deg) translateY(-300px)` }}></div>
          ))}
        </div>
      </div>
      ...
      <div className="relative w-full max-w-6xl h-full flex flex-col md:flex-row items-center justify-between md:justify-center z-20 pointer-events-none py-24 md:py-0 px-6 md:px-0">
        ...
        <div className="flex flex-col w-full md:hidden gap-6">
          {categoryData.technical.specs.slice(0, 2).map((spec, idx) => (
        ...
        <div className="flex flex-col w-full md:hidden gap-6 mt-auto">
          {categoryData.technical.specs.slice(2, 4).map((spec, idx) => (
            <div key={idx} className="flex flex-col items-center text-center w-full">
              <div className="text-[10px] text-white/50 tracking-widest uppercase mb-1">{spec.label}</div>
              <div className="text-xl font-bold text-white border-b-2 border-white/20 pb-1 px-4">{spec.value}</div>
            </div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-30 pointer-events-auto">
        <button 
          ...
          className="px-8 py-4 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold tracking-[0.3em] uppercase text-white hover:bg-white/10 transition-all flex items-center gap-3 group"
        >
